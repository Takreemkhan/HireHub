...
function mapJobToSearchResult(apiJob: any): SearchResult {
  ...
  // A job is "actively featured" only when the featuredUntil timestamp is in the future
  const isFeaturedActive = !!(
    apiJob.isFeatured &&
    apiJob.featuredUntil &&
    new Date(apiJob.featuredUntil) > new Date()
  );
  return {
    id: apiJob._id,
    type: 'project',
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        ...
        // Featured (non-expired) jobs bubble to the very top
        const sorted = [...mappedJobs].sort((a, b) => Number(b.featured) - Number(a.featured));
        setJobs(sorted);
      } catch (err: any) {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div className="lg:col-span-3">
              <div className="space-y-4">
                {[1, 2, 3, 4, 5].map((i) => (
                  <div key={i} className="bg-white border border-gray-100 rounded-xl p-6 shadow-sm overflow-hidden relative">
                    {/* Shimmer sweep overlay */}
                    <div className="absolute inset-0 -translate-x-full animate-[shimmer_1.4s_infinite] bg-gradient-to-r from-transparent via-white/70 to-transparent" style={{ animationDelay: `${i * 0.15}s` }} />
                    {/* Title row */}
                    <div className="flex items-center gap-3 mb-3">
                      <div className="h-6 bg-gray-200 rounded-md animate-pulse" style={{ width: `${55 + (i % 3) * 12}%` }} />
                      <div className="h-5 w-20 bg-orange-100 rounded-md animate-pulse" />
                    </div>
                    {/* Budget + NEW badge row */}
                    <div className="flex items-center gap-2 mb-3">
                      <div className="h-4 w-28 bg-gray-200 rounded animate-pulse" />
                      <div className="h-4 w-10 bg-blue-100 rounded animate-pulse" />
                    </div>
                    {/* Description lines */}
