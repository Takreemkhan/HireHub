...
export default function ResumeVideoManager({ readOnly = false, targetUserId }: Props) {
  ...
  const handleUpload = async (files: File[]) => {
    ...
    if (slots <= 0) {
      setError(`Maximum ${MAX_VIDEOS} videos. Delete one first.`);
      return;
    }
  ...
  const dropZoneClass = `relative cursor-pointer rounded-xl border-2 border-dashed transition-all duration-200 p-8 text-center ${
    isDragging
    ...
    <>
      <div className="bg-white rounded-2xl border border-gray-100 shadow-lg overflow-hidden transition-all duration-200">
        ...
        <div className="p-6">
                  ...
                  {!readOnly && editingId !== video.id && (
                    <div className="flex flex-col sm:flex-row gap-1 flex-shrink-0">
                      ...
                      <button
                        ...
                        disabled={uploading}
                        className="p-2 rounded-lg text-green-600 hover:bg-green-50 transition disabled:opacity-40"
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {!readOnly && videos.length < MAX_VIDEOS && (
            <div
                  ...
                  {isDragging ? (
                    ...
                    <>
                      ...
                      <p className="text-xs text-gray-400">
                        MP4, WebM, MOV, AVI · Max {MAX_MB}MB · {MAX_VIDEOS - videos.length} slot
                        {MAX_VIDEOS - videos.length !== 1 ? "s" : ""} left
                      </p>
                    </>
                  )}
                </div>
