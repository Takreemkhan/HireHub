...
export default function OverviewSection() {
  ...
  const openProposalId = searchParams.get('openProposal');

  // Auto-open modal on redirect callback constraint
  useEffect(() => {
    if (openProposalId && !proposalJob) {
      fetch('/api/jobs/' + openProposalId)
        .then((r) => r.json())
        .then((data) => {
          if (data.job) {
            setProposalJob(mapApiJobToCard(data.job));
            // Optionally strip searchParams from URL to prevent infinite reopening on refresh
            router.replace('/freelancer-dashboard');
          }
        })
        .catch(console.error);
    }
  }, [openProposalId, router, proposalJob]);
      ...
      <div className="max-w-7xl mx-auto space-y-10 pb-10">
        ...
        <div className="bg-white border border-[#E2E8F0] rounded-2xl p-5 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-baseline gap-2">
                ...
                <span className="text-sm text-[#6B7280]">Bids remaining</span>
              </div>
              ...
              {bitsRemaining !== null && bitsRemaining === 0 && (
                <p className="text-xs text-red-500 font-semibold mt-1">🚫 No Bids left – buy more to keep bidding</p>
              )}
            </div>
          </div>
          <div className="flex flex-col sm:items-end gap-2">
            ...
            >
              {bitsRemaining === 0 ? 'Get More Bids →' : 'Manage Bids →'}
            </button>
          </div>
        </div>
