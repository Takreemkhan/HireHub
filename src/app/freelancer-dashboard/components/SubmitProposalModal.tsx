...
export default function SubmitProposalModal({ job, onClose, onBitUsed }: SubmitProposalModalProps) {
  ...
  const addRecordedVideo = () => {
    ...
    if (resumeVideos.length >= MAX_VIDEOS) {
  ...
  const processVideoFiles = useCallback((files: File[]) => {
    ...
    const remaining = MAX_VIDEOS - resumeVideos.length;
    if (remaining <= 0) {
      setVideoError('Maximum 2 videos allowed. Remove a video to add a new one.');
      return;
    }
    ...
    if (videoFiles.length > remaining) {
      setVideoError(`Only ${remaining} more video${remaining > 1 ? 's' : ''} can be added (max ${MAX_VIDEOS} total).`);
    }
    ...
    try {
      ...
      if (resumeVideoItems.length > 0) {
        const saveRes = await fetch('/api/resume-video', {
          ...
          body: JSON.stringify({ resumeVideos: resumeVideoItems }),
        });
              ...
              <div className="border border-[#E2E8F0] rounded-xl overflow-hidden">
                ...
                <div className="flex items-center justify-between px-4 py-3 bg-[#FAFBFC] border-b border-[#E2E8F0]">
                  <div>
                    ...
                    <span className="ml-2 text-xs text-[#6B7280]">Optional · Max {MAX_VIDEOS} videos · Up to {MAX_VIDEO_SIZE_MB}MB each</span>
                  </div>
                  <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${resumeVideos.length >= MAX_VIDEOS ? 'bg-orange-100 text-orange-600' : 'bg-gray-100 text-gray-500'}`}>
                    {resumeVideos.length}/{MAX_VIDEOS}
                  </span>
                </div>
                ...
                {isPlanActive === false && (
                  <div className="p-6 text-center space-y-3">
                    ...
                    <button
                      type="button"
                      onClick={() => { onClose(); router.push('/freelancer-plans'); }}
                              ...
                              {isDragging ? (
                                ...
                                <>
                                  ...
                                  <p className="text-xs text-[#6B7280]">
                                    MP4, WebM, MOV, AVI · Max {MAX_VIDEO_SIZE_MB}MB · {MAX_VIDEOS - resumeVideos.length} slot{MAX_VIDEOS - resumeVideos.length !== 1 ? 's' : ''} remaining
                                  </p>
                                </>
                              )}
                            </div>
                      ...
                      <div>
                        {resumeVideos.length >= MAX_VIDEOS ? (
                          <div className="text-center py-6 text-sm text-[#6B7280]">
                            <p>Maximum {MAX_VIDEOS} videos reached.</p>
                      ...
                      <div>
                        {profileVideosLoading ? (
                          ...
                          <div className="space-y-2">
                            <p className="text-xs text-[#6B7280] mb-2">Select videos from your profile to attach to this proposal (max {MAX_VIDEOS} total):</p>
                            {profileSavedVideos.map((video, idx) => {
                              const isSelected = selectedProfileVideos.includes(video.url);
                              const canSelect = !isSelected && (resumeVideos.length + selectedProfileVideos.length) < MAX_VIDEOS;
                              return (
                                <div
                                  key={idx}
                                  onClick={() => {
