...
import { CheckCircle, Zap, Star, Crown, Loader2 } from "lucide-react";
...
const PLAN_META: Record<string, {
    ...
    elite: {
        icon: <Crown size={22} />,
        badge: "Best Value",
        badgeColor: "bg-[#1B365D] text-white",
        accentColor: "#1B365D",
        features: [
        ...
        const fetchPlans = async () => {
            try {
                ...
                if (data.success) {
                    ...
                    setKeyId(data.keyId);
                    // Pre-select active plan if exists, else default "pro"
    ...
    const handleBuyPlan = async () => {
        ...
        try {
            ...
            const rzp = new window.Razorpay({
                ...
                theme: { color: PLAN_META[plan.planKey]?.accentColor ?? "#FF6B35" },
                        ...
                        {plans.map((plan) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div
                                        className="h-1.5"
                                        style={{ background: isCurrentActive ? "#22c55e" : meta.accentColor }}
                                    />

                                    {/* Badge */}
                                    {(meta.badge || isCurrentActive) && (
                                        <div className="absolute top-4 right-4">
                                            <span className={`text-[11px] font-bold px-2.5 py-1 rounded-full ${isCurrentActive ? "bg-green-100 text-green-700" : meta.badgeColor
                                                }`}>
                                                {isCurrentActive ? "✓ Active" : meta.badge}
                                            </span>
                                        </div>
                                    )}
                                        ...
                                        <div className="space-y-2.5">
                                            {meta.features.map((f, i) => (
                                                <div key={i} className="flex items-start gap-2">
                                                    ...
                                                    >
                                                        <svg className="w-2.5 h-2.5" style={{ color: meta.accentColor }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
