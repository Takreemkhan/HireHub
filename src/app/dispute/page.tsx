...
export default function DisputeRepostPage() {
  ...
  useEffect(() => {
    if (jobId) {
      ...
      fetch(`/api/milestones?jobId=${jobId}`)
        ...
        .then(data => {
          if (data.success) {
            // Only show milestones that are active, being reviewed, in revision, or released.
            // Filter out 'locked' and 'proposed' milestones as they are not currently in play.
  ...
  const addFiles = useCallback((files: File[]) => {
    ...
    for (const file of files) {
      ...
      if (isVideo && file.size > 100 * 1024 * 1024) continue; // 100MB video limit
      if (isImage && file.size > 10 * 1024 * 1024) continue;  // 10MB image limit
  ...
  // ── Step Validation ────────────────────────────────────────────────────────
  // Requirement: User requested no character limits
  const canProceedStep1 = disputeType !== "" && disputeTitle.trim().length >= 5 && description.trim().length >= 1;
  const canProceedStep2 = true; // media is optional
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 2. Submit the dispute with uploaded media URLs
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                >
                  ...
                  {/* Single button for both images and videos */}
                  <button
                    type="button"
