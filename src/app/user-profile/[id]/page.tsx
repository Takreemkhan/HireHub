...
import ResumeVideoManager from "../../freelancer-dashboard/components/ResumeVideoManager";
import { useGetResumeVideosByUserId, useProfileDetails, useCurrentJobsClients } from "@/app/hook/useProfile";
...
function FreelancerProfilePageContent({ id }: { id: string }) {
  ...
  // React Query Hooks
  const { data: profileQueryData, isLoading: profileLoading, error: profileQueryError } = useProfileDetails(id);
  const profile = apiToPreviewProfile(profileQueryData?.profile || null);
  const rawProfile = profileQueryData?.profile || {};
  const loading = (sessionStatus === "loading") || (!profileQueryData && profileLoading);
  ...
  const { data: savedIds } = useSavedFreelancersIds(!!id && sessionStatus === "authenticated");
  ...
  const handleOpenChatForJob = useCallback(async (jobId?: string) => {
    if (!id || !userId) return;
    setChatLoading(true);
    try {
      const payload: any = { otherUserId: id };
      ...
      queryParams.set("otherUserId", id);
      ...
      router.push(`/client/messages?otherUserId=${id}${jobId ? `&jobId=${jobId}` : ''}`);
    } finally {
      setChatLoading(false);
      setIsHireModalOpen(false);
    }
  }, [id, userId, profile.name, router]);
  ...
  if (!session) return null; // Wait for redirect to trigger
  ...
  const isClientView = !!id && id !== userId;
            ...
            <div className="flex-1 flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left gap-8">
              ...
              <div className="flex-1 text-white py-2">
                ...
                <div className="flex flex-wrap items-center justify-center sm:justify-start gap-x-5 gap-y-2 mt-3 text-sm text-white/80 font-medium">
                  ...
                  {/* EXTRA DETAILS FROM OLD PAGE: Badges */}
                  <span className="flex items-center gap-1.5 ml-2"><span className="bg-blue-100 p-0.5 rounded-full"><svg className="w-3 h-3 text-blue-600" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg></span> <span className="text-white">100% Job Success</span></span>
                  <span className="flex items-center gap-1.5 ml-2"><span className="bg-blue-600 p-0.5 rounded-full"><svg className="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg></span> <span className="text-blue-400">Top Rated</span></span>
                </div>
                ...
                {isClientView && (
                  <div className="mt-8 flex flex-wrap justify-center sm:justify-start gap-3">
                    ...
                    {/* Contact Now Button — hidden per design */}
                    ...
                    {/* EXTRA DETAILS FROM OLD PAGE: Share Button */}
                    <button
                      onClick={handleShare}
                      className="p-3 rounded-[1rem] transition shadow-lg flex items-center justify-center bg-white/10 text-white hover:bg-white/20"
                      title="Share Profile"
                    >
        ...
        <div className="max-w-7xl mx-auto px-6 py-12 md:pt-16 pb-20">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8 min-w-0">
              ...
              {/* EXTRA DETAILS FROM OLD PAGE: Platform Work History (CompletedJobs) */}
              <div className="bg-white rounded-3xl shadow-sm p-8 border border-gray-100/50">
                <h2 className="text-2xl font-bold mb-6" style={{ color: "#1B365D" }}>Platform Work History</h2>
                <div className="border border-gray-100 rounded-2xl overflow-hidden p-2">
                  <CompletedJobs
                    completedJobsList={rawProfile.completedJobsList || []}
                    activeJobsList={rawProfile.activeJobsList || []}
                    completedCount={rawProfile.completedJobs ?? 0}
                    inProgressCount={rawProfile.activeProjects ?? 0}
                  />
                </div>
              </div>

              {/* Resume Videos — moved up, right after Platform Work History */}
              <div id="video-manager-section" className="bg-white rounded-3xl shadow-sm p-8 border border-gray-100/50">
                ...
                <ResumeVideoManager readOnly={true} targetUserId={id || undefined} />
              </div>
            ...
            <div className="space-y-8">
              ...
              {/* Education — moved here, below Languages */}
              {profile.education.length > 0 && (
                <SideCard title="Education">
                  <div className="space-y-4">
                    {profile.education.map((edu, idx) => (
                      <div key={idx} className="flex gap-4 items-start p-4 rounded-2xl bg-gray-50/50 border border-gray-100 transition hover:shadow-md hover:bg-white">
                        <div className="w-10 h-10 bg-green-50 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm border border-green-100">
                          <svg className="w-5 h-5 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path d="M12 14l9-5-9-5-9 5 9 5z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 14l9-5-9-5-9 5 9 5zm0 0l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0112 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14zm-4 6v-7.5l4-2.222" />
                          </svg>
                        </div>
                        <div>
                          <h3 className="font-bold text-sm" style={{ color: "#1B365D" }}>{edu.degree}</h3>
                          <p className="text-[13px] font-medium text-gray-600 mt-0.5">{edu.school}</p>
                          <p className="text-xs text-gray-400 mt-1 uppercase tracking-wider font-semibold">{edu.period}</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {/* Professional Experience — moved below Education */}
              {profile.experience.length > 0 && (
                <SideCard title="Experience">
                  <div className="space-y-4">
                    {profile.experience.map((exp, idx) => (
                      <div key={idx} className="flex gap-4 items-start p-4 rounded-2xl bg-gray-50/50 border border-gray-100 transition hover:shadow-md hover:bg-white">
                        <div className="w-10 h-10 bg-blue-50 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm border border-blue-100">
                          <svg className="w-5 h-5 text-[#2E5984]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                          </svg>
                        </div>
                        <div>
                          <h3 className="font-bold text-sm" style={{ color: "#1B365D" }}>{exp.title}</h3>
                          <p className="text-[13px] font-semibold mt-0.5" style={{ color: "#FF6B35" }}>{exp.company}</p>
                          <p className="text-xs text-gray-400 mt-1 uppercase tracking-wider font-semibold">{exp.period}</p>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#1B365D]/40 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-100 animate-in zoom-in-95 duration-200">
              ...
              {loadingJobs ? (
                ...
                <div className="space-y-2 max-h-[280px] overflow-y-auto pr-2 custom-scrollbar">
                  {clientJobs.map((job: any) => (
                    <label key={job._id} className={`flex items-center justify-between gap-3 p-2.5 px-4 rounded-xl border-2 cursor-pointer transition-all duration-200 ${selectedJobId === job._id ? 'border-[#FF6B35] bg-orange-50/30 shadow-sm' : 'border-gray-100 hover:border-[#2E5984]/30 hover:bg-gray-50'}`}>
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        <input type="radio" name="jobSelect" value={job._id} checked={selectedJobId === job._id} onChange={() => setSelectedJobId(job._id)} className="w-4 h-4 text-[#FF6B35] focus:ring-[#FF6B35] border-gray-300 flex-shrink-0" />
                        <p className="font-bold text-[#1B365D] truncate text-[15px]">{job.title}</p>
                      </div>
                      <span className="text-xs font-bold bg-white px-2.5 py-1 rounded-lg text-green-700 border border-green-200 shadow-sm flex-shrink-0">
                        Budget: ${job.budget}
                      </span>
                    </label>
                  ))}
            ...
            <div className="p-5 border-t border-gray-100 bg-gray-50/50 flex justify-end gap-3">
              ...
              <button 
                onClick={() => selectedJobId ? handleOpenChatForJob(selectedJobId) : undefined}
...
export default function FreelancerDetailPage({
  ...
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center font-bold text-[#1B365D] text-lg gap-3"><div className="w-6 h-6 border-3 border-[#FF6B35] border-t-transparent rounded-full animate-spin" /> Loading Profile...</div>}>
      <FreelancerProfilePageContent id={id} />
    </Suspense>
  );
}