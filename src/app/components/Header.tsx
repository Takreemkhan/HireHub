...
const Header = ({ className = "" }: HeaderProps) => {
  ...
  const getMoreDropdownItems = () => [
    { name: "Dashboard", path: isClient ? "/client-dashboard" : "/freelancer-dashboard", icon: "UserCircleIcon" },
    { name: "Payment Center", path: "/payment-center", icon: "CreditCardIcon" },
  ];
  ...
  const moreDropdownItems = session?.user ? getMoreDropdownItems() : [];
      ...
      <div className="w-full">
        <div className="flex items-center justify-between h-16 px-4 lg:px-8">
          ...
          <nav className="hidden lg:flex items-center space-x-1">
            ...
            {session?.user && (
              <div className="relative group">
                <button className="flex items-center space-x-2 px-4 py-2 rounded-lg font-body font-medium text-foreground hover:bg-muted hover:text-primary transition-all duration-300">
                  <Icon name="EllipsisHorizontalIcon" size={20} />
                  <span>More</span>
                </button>
                <div className="absolute right-0 top-full mt-2 w-56 bg-card rounded-lg shadow-brand-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 border border-border">
                  {moreDropdownItems.map((item) => (
                    <Link key={item.path} href={item.path}
                      className={`flex items-center space-x-3 px-4 py-3 hover:bg-muted transition-colors duration-200 first:rounded-t-lg last:rounded-b-lg ${isActivePath(item.path) ? "bg-muted text-primary" : "text-foreground"
                        }`}>
                      <Icon name={item.icon as any} size={20} />
                      <span className="font-body font-medium">{item.name}</span>
                    </Link>
            ...
            {session?.user && (
              <>
                ...
                <div className="pt-4 border-t border-border">
                  <p className="px-4 py-2 text-sm font-semibold text-muted-foreground">More</p>
                  {moreDropdownItems.map((item) => (
                    <Link key={item.path} href={item.path}
                      className={`flex items-center space-x-3 px-4 py-3 rounded-lg font-body font-medium transition-all duration-300 ${isActivePath(item.path)
                        ? "bg-primary text-primary-foreground shadow-sm"
