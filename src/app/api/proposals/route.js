...
export async function POST(req) {
  try {
    ...
    // ── Check active resume video subscription if resumeID is provided ─────────
    if (resumeID) {
      ...
      if (!isPlanActive) {
        return NextResponse.json(
          {
            success: false,
            message: "Your Resume Video plan has expired or is inactive. Upgrade to attach videos to proposals.",
            code: "NO_VIDEO_PLAN",
          },
          { status: 403 }
        );
